...
最後更新：2026-07-25（新增永久備份例外 v02_17：大改版前基準）
...
**永久備份例外：** `v02_17`（2026-07-25 起）——使用者在開始「統一整個程式碼渲染函式」的大改版工作前，明確要求先把當時的版本標註為永久保留，大改版完成前不參與 5 版輪替清理。大改版完成、經使用者確認不再需要保留後，再移除此例外並恢復正常輪替；若使用者之後另外指定新的永久保留基準，依當時指示調整此區塊。
