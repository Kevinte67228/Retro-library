...
**2. 清理舊備份**
`_internal/old/` 最多保留 3 個版本，超過則刪最舊的。

**3. 推送新版檔案**
把新版本的以下檔案推到 `GameVault/`：
- `index.html`
- `GameVault_vXX_YY_index.html`（新版本號）
- `sw.js`
- `manifest.json`
- `GameVault_AppsScript.gs`（有更新時）

並刪除舊的版本 HTML（`GameVault_vXX_YY-1_index.html`）。

**4. 更新 CHANGELOG**
在 `_internal/CHANGELOG.md` 頂端插入新版記錄，超過 3 筆則刪最舊。
...
`_internal/CHANGELOG.md` 記錄每次版本的改動內容，保留最近 3 筆。格式如下：
