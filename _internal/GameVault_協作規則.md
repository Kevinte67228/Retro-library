...
**在依賴這份文件裡任何版本號、CI/CD 狀態、部署流程做判斷之前，務必先用 GitHub Contents API 重新抓取這份文件、`GameVault/index.html` 的 `APP_VERSION`、`GameVault/GameVault_AppsScript.gs` 開頭版本註解的最新內容，不要只憑對話一開始拿到的快照或先前對話摘要判斷現況。**
...
└── _internal/              ← 不部署（私有，Netlify 完全不碰）
    ...
    ├── GameVault_協作規則.md
    ├── GameVault_部署架構說明.md  ← Claude存取GitHub/GAS自動部署/Netlify自動部署的技術說明
    ├── gh_batch.py          ← Git Data API 批次操作 helper（單一commit完成多檔異動，不含 token）
    ├── github_deploy.py     ← 部署腳本（呼叫 gh_batch.py，備份+輪替+部署包在同一次commit）
...
- `GameVault_AppsScript.gs` 放在 `GameVault/` 根目錄，讓使用者可從 App 內直接下載（連結加 `download` 屬性），同時也是 CI/CD 自動部署的來源檔案。
...
**已於 2026-07-06 設定完成並驗證成功。** 推送 `GameVault/GameVault_AppsScript.gs` 到 GitHub `main` 分支時，會自動觸發 GitHub Actions，用 [clasp](https://github.com/google/clasp) 把程式碼推上 Google Apps Script，並更新到**固定的 Web App 部署**（deployment），網址不會改變。**取代了先前「使用者手動貼到 Apps Script 編輯器」的流程**，Claude 推送 `.gs` 到 GitHub 後即完成後端部署，不需要再請使用者手動操作。

### 運作方式

- Workflow 檔案：`.github/workflows/deploy-gas.yml`
- 觸發條件：push 到 `main` 且異動 `GameVault/GameVault_AppsScript.gs`；也支援手動觸發（`workflow_dispatch`）
...
## GitHub 批次操作（Git Data API，多檔案異動的標準做法）

**已於 2026-07-17 確立為標準做法。** 任何一次涉及**多個檔案**的新增／修改／刪除（備份、清理輪替、多檔部署等），一律改用 **Git Data API**（Trees／Commits／Refs）打包成**單一 commit**，不要逐檔案呼叫 Contents API（`PUT`／`DELETE /contents/...`）。

**為什麼**：Contents API 是「一個檔案一個 commit」，每次呼叫都是完整的 HTTP 請求＋Git commit，實測每次約 2-3 秒；檔案一多，總耗時等於「檔案數 × 2-3 秒」。改用 Git Data API 後，不管異動幾個檔案，固定只需要約 4 次 API 呼叫（讀目前 tree → 建新 tree → 建 commit → 更新 ref），實測 3-8 個檔案的操作約 1.3-2.5 秒完成，且歷史紀錄更乾淨（一次異動只留一筆 commit，而不是一堆逐檔案的小 commit）。

**流程：**
1. `GET /git/ref/heads/main` 拿目前分支指向的 commit sha
2. `GET /git/commits/{sha}` 拿該 commit 的 tree sha
3. 若有新增/修改檔案：先各自 `POST /git/blobs`（base64 內容）取得 blob sha
4. `POST /git/trees`，帶 `base_tree`（上面拿到的 tree sha）＋ `tree` 陣列：
   - **新增/修改**：`{"path":..., "mode":"100644", "type":"blob", "sha": blob_sha}`
   - **刪除**：`{"path":..., "mode":"100644", "sha": null}`　⚠️ **刪除時絕對不能帶 `type` 欄位**，帶了會得到 `422 GitRPC::BadObjectState`；這是唯一容易踩雷的地方。
5. `POST /git/commits`，帶 `message`、`tree`（上一步新 tree 的 sha）、`parents`（步驟1的 commit sha）
6. `PATCH /git/refs/heads/main`，帶 `sha`（新 commit sha）完成更新

新增與刪除可以混在同一次 `tree` 陣列裡，一次 commit 同時完成（已測試驗證）。

**適用時機：** 備份輪替（複製多檔到 `_internal/old/vXX_YY/`）、清理舊備份（刪一整個版本資料夾）、一次部署多個檔案（`index.html` + 版本化HTML + `sw.js` 等）都應該用這個方式包成一次操作。單一檔案的小修改（例如只改一行 CHANGELOG）用 Contents API 即可，不必為了單檔案特地走批次流程。

**現成工具：** `_internal/github_deploy.py`（呼叫 `gh_batch.py`）已把「備份目前版本 → 清理超過5版的最舊備份（跳過永久例外）→ 推送新版檔案 → 刪除舊版本HTML」整個流程包進**同一次 commit**。備份時用 `build_path_index()` 直接複製既有 blob sha，不需要重新下載/上傳內容，更快。用法：`python3 github_deploy.py <新版本號> <本機資料夾路徑>`（需先 `export GH_TOKEN=ghp_...`）。
