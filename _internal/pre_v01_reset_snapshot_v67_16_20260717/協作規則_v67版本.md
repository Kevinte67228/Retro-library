...
- 封存用：`GameVault_v67_AppsScript.gs.txt`
...
**v67.01 ～ v67.16 期間主要更新（純前端，GAS 未動）：**
- **市場管理**（設定頁）：新增「🔀 市場顯示設定」（勾選顯示/隱藏各查價市場、↑↓ 調整順序，`cfg.marketHidden`／`cfg.marketOrder`）；估值查詢依商品分類自動加入限定詞（`MARKET_CAT_KEYWORD`／`marketQueryQualifier`），台站用中文、日站自動切日文限定詞。
- **CSV 匯出/匯入大改**：改為**雙標題列格式**（第一列內部鍵值、第二列中文顯示名、資料從第三列起）；修正原本用中文顯示名當標題導致匯入欄位對不上的 bug；圖片欄位正常匯出（存的是 Drive 檔案 ID 短字串）；匯入改為**逐筆寫回雲端試算表**（`_syncImportedRecords`，修正原本只寫本機快取、重整後資料消失的資料遺失問題）；`FileReader` 補 `onerror`（修正選檔後無反應）。
- **尋寶頁匯出/匯入**：比照收藏頁，工具列的同步 icon 併入「⋯」選單（從雲端同步／匯出 CSV／匯入 CSV），大量刪除維持獨立；新增 `exportHuntCsv()`；匯入沿用既有 `triggerImportCsv()`（自動偵測「狩獵」分類），完成後補呼叫 `renderHuntList()`。
- **程式碼清理**：移除 2 處診斷 `console.log`；儀表板各自重複定義的 `fmt()`（5 次）/`catKey()`（2 次）抽成頂層共用 `_dashFmt()`／`_dashCatKey()`。全碼掃描結論：無孤立死函式、無註解掉的程式碼、CSS 無未使用 class。
- **預設樣本更新**：`SAMPLE_ITEMS` 改用實際匯出的遊戲 5 筆、新增 `SAMPLE_HUNT_ITEMS` 尋寶 6 筆；`loadSamples()` 改為同時載入收藏（col）與尋寶（hunt），保留原始 uuid（關聯連結有效）＋ uuid 去重防護。注意：舊樣本用的欄位鍵名（`media_type`/`version`/`voice_lang` 等）與實際內部鍵名不符，新資料一律採 CSV 匯出的真實鍵名（`media`/`edition`/`voice_language` 等）。
- **使用手冊（`manual.html`）全面重寫**：改為 16 章節結構，圖片改用 GitHub raw URL 引用（`GameVault/Photo/NNNN.jpg`），不再內嵌 base64。截圖以 4 碼編號佔位、由使用者分批提供後合併。已知待補：iOS「加入主畫面」目前無配圖；第 14 章統計儀表板文字尚未與實際畫面完整核對。
...
- 兩次都是**依規則產生的正常歸零**，不代表往後每次 GAS 有任何異動都要重整——見下方「主版號（XX）異動條件」對「實質變更」的定義。

**永久備份例外：** `_internal/old/v42_20a1/`、`_internal/old/v67_01/` 是使用者要求的永久備份，5 版輪替清理時一律跳過，除非使用者明確要求刪除。（永久基準隨每次版號重整更新命名：`v54_48`→`v54_70`→`v66_01`→`v67_01`。）
...
1. 在 Claude 容器建立新版本資料夾，例如 `/home/claude/v67_17/`。
2. 修改 `index.html` 與 `GameVault_v67_17_index.html`（兩者內容完全一致）。
3. 更新 `APP_VERSION='v67.17'`。
4. 更新 `sw.js` 的 `CACHE_NAME`，例如 `gamevault-v67-17`。
...
2. 更新 `APP_VERSION='vXX.YYaN'`（如 `v67.17a1`）。
